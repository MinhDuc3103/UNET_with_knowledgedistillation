...
dir_checkpoint = Path('./stu_with_kd/')

teacher_weights = './MODEL.pth'
...
def train_model(
    ...
    # Get teacher output
    teacher_outputs = fetch_teacher_outputs(teacher, train_loader,device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for i, batch in enumerate(train_loader):
                ...
                teacher_output = teacher_output.to(device=device, dtype=torch.long)

                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    output = model(images)
                    loss = loss_fn_kd(output, teacher_output, true_masks)
...
if __name__ == '__main__':
    ...
    teacher = UNet(n_channels=3, n_classes=args.classes, bilinear=args.bilinear)
